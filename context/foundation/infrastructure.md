...
| SQL firewall rule `AllowAzureServices` (0.0.0.0) admits network traffic from Azure-hosted resources in any tenant, not only ours | Implementation review (account-store-foundation Phase 3, 2026-09-25) | L | M | Accepted: the server is Entra-only (no SQL password to guess), the app and deploy identities have least-privilege database users, and VNet integration or a private endpoint is not available on F1 without extra cost. Revisit when moving to B1 (VNet integration) |
| A `gh-runner-<run_id>` SQL firewall rule is left open because the runner was lost mid-deploy, before the `if: always()` cleanup step ran | Implementation review (account-store-foundation full plan, 2026-09-25) | L | L | Accepted: the cleanup step covers failed and cancelled steps (shown on run 36137028290) but not a vanished runner. Check `az sql server firewall-rule list -g rg-battlemap -s <server>` occasionally; only `AllowAzureServices` should remain. Delete any stale `gh-runner-*` rule |
